...
const DoctorDashboard = () => {
  ...
  const savePrescription = async (type) => {
    if (!uid) {
      setError('Please enter a patient UID first.');
      return;
    }

    if (type === 'diabetes') {
      // Check if at least one medicine is prescribed
      const hasMedicine = Object.values(diabetesPrescription).some(value => value !== '');
      if (!hasMedicine) {
        setError('Please specify at least one diabetes medication.');
        return;
      }
    } else if (type === 'general' && !generalPrescription.trim()) {
      setError('Please enter a general prescription.');
      return;
    }
    ...
    try {
      const newPrescription = {
        ...
        ...(type === 'diabetes' ? diabetesPrescription : { generalPrescription })
      };
      ...
      if (type === 'diabetes') {
        ...
        setGeneralPrescription('');
      }

      // Refresh recent prescriptions
      setRecentPrescriptions([response.data.prescription, ...recentPrescriptions]);
      setError('');
    } catch (error) {
